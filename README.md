...
- 当前下一步：执行 [PG-P0-07 systemd 常驻服务与 Guardian 自身保护](docs/27-production-guardian-roadmap.md#pg-p0-07-systemd-常驻服务与-guardian-自身保护)；PG-P0-06 的本地 MVP 证据见 [EXP-033](experiments/EXP-033-2026-09-20-two-layer-recovery/record.md)。此前不开始新的 `enforce`、restart、terminate 或生产接入。G6-T06 仍需 leader 评审后才能实现旁路 endpoint。
...
从 [docs/27 状态板](docs/27-production-guardian-roadmap.md#10-当前状态板) 依次推进；PG-P0-01/02/03/04/05/06 已完成，当前从 `PG-P0-07` systemd 常驻与自身保护开始。在 P0-01–07 全部通过且用户对当次本地 disposable 对象单独授权前，不执行新的真实动作。
