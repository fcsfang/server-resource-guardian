...
- 当前下一步：依据 [Beszel 字段清单](docs/22-beszel-dashboard-field-inventory.md) 完成登录态页面、历史数据和真实告警核验，随后用 [事件契约](docs/21-beszel-guardian-event-contract.md) 推进只读 Adapter 的实际脱敏 payload 映射；生产测试授权、保护名单和 x86_64 兼容性复核仍待外部条件。
