...
PG-P0-01、PG-P0-02 和 PG-P0-03 已完成；当前首个可执行任务是 **PG-P0-04：每对象归因与放弃机制**。本文的状态板和 Goal 7 必须与代码、实验记录同步更新。
...
| PG-P0-04 对象归因 | `READY` | P0-03 | 当前仍仅单容器启发式，尚无贡献度/置信度/歧义放弃 | 先建立对象 registry 和 Docker↔cgroup 映射 |
| PG-P0-05 策略/授权/耐久性 | `BACKLOG` | P0-02,P0-04 | ledger 无事务/并发安全 | 等待依赖 |
