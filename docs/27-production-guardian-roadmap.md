...
| PG-P0-07 systemd/自身保护 | `IN_PROGRESS` | P0-03,P0-05,P0-06 | EXP-034/035/036/037/038/040/041/043/044/045/046/047/048/049：主机全量 `129/129`，审计 `flush/fsync` 与完整性校验正负向契约通过；当前 Observer/Runtime 相关 VM 回归 `25/25`，审计校验器 VM 回归 `4/4`，并以当前代码完成两次 VM `observe --once` smoke 和 45 秒连续 observe soak（7/7 审计写入、无动作、最大 RSS 27,612 KiB）；EXP-048 以自然退出码 `137` 模拟一次异常退出并验证 transient restart counter `1`→第二轮 success；既有隔离基线 `78/78`；systemd verify 退出码 0、8 秒 smoke、有界存储 fail-closed、45 秒/5 分钟局部 soak、依赖 fixture、transient readiness/watchdog 通知接收、watchdog 余量契约、间隔覆盖 fail-closed、有限高压采样和自然失败重启；EXP-042 预检失败已保留；EXP-039 运行中约 7,219 秒、sidecar 706 样本/7,071 秒，RSS P95/P99/最大值均为 17,752 KiB，CPU/FD/线程 P99 为 0.0%/5/1，RSS/FD 均值为 17,495.354 KiB/3.609，审计 1,046,557 bytes 且 136/136 行合法 JSON、事件 ID 无重复，Observer 存活；24h soak、watchdog 超时、真实 SIGKILL/OOM 恢复、磁盘满边界和完整场景 P99 未完成；EXP-039 启动早于 EXP-047，不代表新持久写入契约已在长跑中验证 | 检查 EXP-039 结束结果，继续只读日志容量统计、剩余故障边界和资源 P99 测量；长跑结束后用当前代码重新验证相关窗口 |
