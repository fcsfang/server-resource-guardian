...
当前状态：**Hub/Agent 已部署，认证连接已验证；G6-T01～G6-T04 已完成，EXP-028 已从本地 `data.db` 只读确认两次压力告警和一次低阈值正向控制的恢复历史；G6-T05 bridge 已完成，当前进入 G6-T06 组长评审交付准备。**
...
1. **G6-T06**：使用 [一页式效果对照](24-guardian-effectiveness-one-page.md)、[离线演示页](../demo/guardian-beszel-review/index.html) 和 [现场 Runbook](26-guardian-beszel-live-demo-runbook.md) 完成组长评审；正式评审通过前不修改 Beszel 上游、不实现生产 endpoint。
2. 按 [`Goal 6`](../goals/resource-protection.md#goal-6beszel-二次开发集成) 和 [`docs/16`](16-autonomous-execution-roadmap.md) 保持 Beszel 观测与 Guardian 决策解耦。
3. 不要把 Beszel 告警直接等同于自动处置授权；`observe/simulate` 证据完成前不进入新的 `enforce` 实验。
