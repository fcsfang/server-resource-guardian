...
推荐方案是“预防 + 成熟监控栈 + 受控本机处置”，而不是单独开发一个自动杀进程程序：

1. 优先给业务进程和容器设置 cgroup/Docker 资源边界，从源头避免单个工作负载拖垮宿主机。
...
6. 只有现有工具无法满足“审批、保护名单、防重放、审计、结果验证”时，才开发轻量 Guardian。
...
当前最合理的下一步不是编写自动杀进程代码，而是执行 [生产环境信息采集](09-production-discovery.md)，再按生产版本建立 WSL2 测试环境并运行 [PoC 验证蓝图](07-poc-blueprint.md)。先用 Beszel 完成只读监控、Docker/systemd 归因和告警，再补充 PSI 与现场快照。PoC 数据能够证明缺口后，再决定使用 Monit/systemd-oomd、接入现有远程运维平台，还是开发 Guardian。
