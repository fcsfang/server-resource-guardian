...
- 宿主机测试 47/47 通过；Multipass Ubuntu 内 Adapter 测试 10/10 通过；真实本地 Hub 健康 GET 返回 200。
...
本实验可以证明 Adapter 的输入安全契约和本地 fixture 行为满足当前设计，并覆盖了已从前端 bundle 识别的 alerts_history 记录形状，但不能证明 Beszel 0.19.0 的真实告警 payload 字段已经完成映射。真实 payload、分页/轮询方式和控制台告警路径仍需在取得本地 Beszel 登录会话后补齐；在此之前不进入 G6-T04 的告警路径对照。
