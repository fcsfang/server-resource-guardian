...
本记录本身仍保留 `INCONCLUSIVE`，因为它还包含单点开销和页面延迟的未闭合限制；G6-T01 的 systemd 缺失项随后由 EXP-026 独立完成验收，真实告警触发/恢复延迟属于 G6-T04。
...
- 23:51 再次检查登录态 UI：命令搜索输入 `service` 返回“未找到结果”，首页“服务”列在当前系统行没有可读数值；因此本地 Beszel 控制台没有提供可直接复查的 systemd 服务明细入口，不能用静态字段清单替代实际页面值。
...
1. systemd 服务表的具体服务当前值、峰值和更新时间尚未在登录态 UI 中独立枚举；命令搜索没有 service 页面，首页服务列也没有可读数值，已有底层 systemd 运行态和静态字段基线不能冒充页面数值证据。
...
- G6-T01 已由 EXP-023、EXP-025 和 EXP-026 合并完成字段可见性、缺失项、更新间隔和运行态开销验收；继续在 G6-T03 处理无 systemd 记录的 fail-closed 边界，G6-T04 再做经授权的告警路径对照。
