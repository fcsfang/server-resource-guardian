...
下一步不再做长时本地内存压力实验。先提交并获得批准的 [x86_64 非生产验证申请包](docs/nonproduction-x86_64-validation-request.md)；获批后先完成 x86_64 安装、只读观察和真实通知送达，再单独申请一次可丢弃容器的 `graceful_stop` 验收。在审批前不连接非生产或生产服务器。
