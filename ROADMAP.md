...
### 当前状态：功能代码已收口；CPU 与磁盘本地闭环已验收，内存真实止损尚未成功验收
...
本地已经完成的能力包括 CPU 观察/模拟与一次本地受控止损闭环、内存观察/模拟、Guardian 自有预留空间的固定范围恢复、保护对象展示、root-only 审计和默认关闭的自动动作。2026-09-22 内存验收已获得针对指定本地可丢弃容器的单次授权，但执行前再次确认时风险证据已不足，Guardian 因此正确拒绝动作；该结果不能算作内存真实止损成功。同日已修正温和停止语义：自动路径只发送一次 `SIGTERM`，超时只转人工，永不自动升级为 `SIGKILL`。磁盘当前只释放 Guardian 自有预留空间并转人工，不是磁盘写入根因自动清理。真实业务恢复、x86_64 非生产和生产环境仍未验证。
...
下一步不再做长时本地内存压力实验。已准备一个未启动、有资源上限且可响应 `SIGTERM` 的新本地容器，获得针对其完整 ID 的单次授权后执行一次短时统一验收。同时提交并获得批准的 [x86_64 非生产验证申请包](docs/nonproduction-x86_64-validation-request.md)；获批后先运行只读准入检查，再完成 x86_64 安装、只读观察和真实通知送达。在审批前不连接非生产或生产服务器。
