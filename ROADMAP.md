...
下一步不再做新的本地压力实验。先完成真实管理员通知渠道配置和送达验收，同时提交并获得批准的 [x86_64 非生产验证申请包](docs/nonproduction-x86_64-validation-request.md)；获批后先运行只读准入检查，再完成 x86_64 安装、只读观察和真实通知送达。在审批前不连接非生产或生产服务器。
