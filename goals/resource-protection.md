...
- [ ] **G4-T05**：在可丢弃测试对象上实现并验证 `enforce` 的受控动作适配器，优先优雅停止/重启，终止动作必须显式授权。授权校验、Docker 参数适配器和 mock executor 已完成；真实动作等待明确的本地可丢弃对象授权。
- [ ] **G4-T06**：完成恢复验证、冷却、失败升级和误报测试；纯逻辑恢复/冷却模型已完成并由 EXP-008 验证，真实动作后的恢复和多对象场景仍待授权实验。
