...
- [ ] **G4-T05**：在可丢弃测试对象上实现并验证 `enforce` 的受控动作适配器，优先优雅停止/重启，终止动作必须显式授权。授权校验、Docker 参数适配器、mock executor 和 `GuardianController` 已完成；真实动作等待明确的本地可丢弃对象授权。
- [ ] **G4-T06**：完成恢复验证、冷却、失败升级和误报测试；纯逻辑和 mock/fake 控制路径已由 EXP-008/009 验证，真实动作后的恢复、多对象竞争和业务健康检查仍待授权实验。
