...
- [ ] **G7-T07 / PG-P0-07**：实现 systemd 常驻服务、watchdog、独立 slice、资源预留/上限和有界日志快照。（进行中：unit/slice、readiness/watchdog、静态校验、有界存储、依赖 fixture、transient notify/watchdog 通知接收、watchdog 状态 fail-closed、有限高压采样、自然失败重启、异常退出码重启、45 秒/5 分钟局部 observe、当前代码连续 observe、审计 `flush/fsync` fail-closed、ENOSPC 负向 fixture 和只读完整性校验器已完成；EXP-039 已按用户优先级在 9,508 秒提前结束，最终 sidecar 为 933 样本/9,346 秒，审计经 EXP-049 校验为 136/136 合法且事件 ID 无重复；该结果是阶段性稳定性证据，不构成 24 小时通过。24 小时 soak、watchdog 超时、真实 SIGKILL/OOM 恢复、真实磁盘满边界、其他真实故障边界与完整场景 P99 校准未完成，EXP-042 的高压预检失败已保留，见 [EXP-034](../experiments/EXP-034-2026-09-20-systemd-runtime-baseline/record.md)、[EXP-038](../experiments/EXP-038-2026-09-20-extended-observer-soak/record.md)、[EXP-039](../experiments/EXP-039-2026-09-20-24h-observer-soak/record.md)、[EXP-041](../experiments/EXP-041-2026-09-20-systemd-watchdog-notify/record.md)、[EXP-042](../experiments/EXP-042-2026-09-20-bounded-high-pressure-sampling/record.md)、[EXP-043](../experiments/EXP-043-2026-09-20-bounded-high-pressure-sampling/record.md)、[EXP-044](../experiments/EXP-044-2026-09-20-systemd-crash-restart/record.md)、[EXP-047](../experiments/EXP-047-2026-09-20-audit-fsync-fail-closed/record.md)、[EXP-048](../experiments/EXP-048-2026-09-20-systemd-abnormal-exit-restart/record.md)、[EXP-049](../experiments/EXP-049-2026-09-20-audit-integrity-verifier/record.md)、[EXP-050](../experiments/EXP-050-2026-09-20-watchdog-status-fail-closed/record.md)、[EXP-051](../experiments/EXP-051-2026-09-20-watchdog-status-observe-soak/record.md)、[EXP-052](../experiments/EXP-052-2026-09-20-enospc-fail-closed/record.md) 和 [docs/32](../docs/32-guardian-systemd-runtime.md)。）
...
- 2026-09-20：PG-P0-07 运行中增量复核 EXP-039；Observer 约 3,701 秒、sidecar 355 样本/3,551 秒，RSS/CPU/FD/线程 P99 为 17,492 KiB/0.0%/5/1，审计仍稳定在 1,046,557 bytes，`verification.json` 保持 `RUNNING`，24 小时完成门和剩余故障边界未改变。
- 2026-09-20：PG-P0-07 再次增量复核 EXP-039；Observer 约 4,564 秒、sidecar 441 样本/4,414 秒，RSS/CPU/FD/线程 P99 为 17,492 KiB/0.0%/5/1，RSS/FD 均值为 17,405.397 KiB/3.603，审计仍稳定在 1,046,557 bytes，`verification.json` 保持 `RUNNING`，24 小时完成门和剩余故障边界未改变。
- 2026-09-20：PG-P0-07 再次增量复核 EXP-039；Observer 约 5,713 秒、sidecar 556 样本/5,567 秒，RSS P99/最大值升至 17,752 KiB，CPU/FD/线程 P99 为 0.0%/5/1，RSS/FD 均值为 17,426.115 KiB/3.619，审计仍稳定在 1,046,557 bytes；该中途资源变化已记录，24 小时完成门和剩余故障边界未改变。
- 2026-09-20：完成 EXP-045；将 Observer unit 的 `WatchdogSec` 从 60 秒调整为 90 秒，写入严格大于配置最大 60 秒采样间隔的回归契约；主机全量测试 121/121 通过。该项只验证模板余量，未触发 watchdog 超时或验证超时恢复。
...
- 2026-09-20：再次只读复核 EXP-039；Observer 约 8,568 秒，sidecar 839 样本/8,404 秒，RSS P95/P99/最大值 17,752 KiB、均值 17,536.038 KiB，CPU/FD/线程 P99 为 0.0%/5/1，审计 136/136 行合法 JSON且事件 ID 无重复；`verification.json` 继续为 `RUNNING`，不改变 24 小时完成门。
- 2026-09-20：完成 EXP-049；新增只读审计完整性校验器，主机全量 `129/129`、Multipass 定向 `4/4` 通过；EXP-039 当前审计快照为 `136/136` 合法 JSON、事件 ID 无重复、容量未超限，校验过程未改写输入；不改变 24 小时、故障恢复和 P99 完成门。
- 2026-09-20：完成 EXP-050；Observer 记录 watchdog `sent/failed/not_configured` 状态，已配置但发送失败时强制 `escalate/not_executed`；主机全量 `132/132`、Multipass 相关回归 `28/28`，当前代码 VM smoke 和 systemd 静态校验通过；未触发真实 watchdog 超时，不改变长跑、故障恢复和 P99 完成门。
- 2026-09-20：完成 EXP-051；当前代码独立 45 秒 observe-only soak 按预设返回 `124`，输出/审计各 7 条，事件 ID 序列一致，stdout watchdog 为 `not_configured`，审计校验 `7/7` 合法；明确 stdout 事后状态字段不回写 JSONL，EXP-039 Observer/sidecar 仍存活。
- 2026-09-20：完成 EXP-052；审计和快照写入注入 `ENOSPC` 时分别返回 `False`/`None`，不抛异常且不覆盖历史；主机全量 `134/134`、Multipass Observer/Runtime `30/30` 通过。该项是负向 fixture，不等同于真实磁盘耗尽恢复。
- 2026-09-20：按用户优先级提前结束 EXP-039；Observer 以正常 `SIGTERM` 收尾，运行 `9,508` 秒，sidecar `933` 个样本/`9,346` 秒，RSS P99/最大值 `17,752 KiB`、均值 `17,557.796 KiB`，审计 `136/136` 条合法且事件 ID 无重复。实验改为 `STOPPED`，24 小时长跑不构成通过；当前主线先转向核心内存危机有效性对照，真实 disposable `graceful_stop` 仍须当次明确授权。
- 2026-09-20：完成 EXP-053 当前代码核心内存危机模拟门禁；全量回归 `134/134`、定向模拟 `4/4`。唯一对象生成 `graceful_stop/not_executed`，多对象/保护对象/归因未确认均 fail-closed；该证据只证明决策门，不证明新的真实动作效果。
