...
- [ ] **G6-T04**：同一可丢弃故障场景下，对照 Beszel 告警路径与 Guardian 本机检测路径，测量检测延迟、漏报、误报、数据中断和降级行为。EXP-028 已完成两次有界内存运行和一次 1%/1 分钟 idle baseline：Guardian 约 6.1 秒发现并恢复，Hub 健康且 PSI full 为 0；Beszel 未产生 active/history 事件，结果为 `INCONCLUSIVE`。已追加 `v0.19.0` 上游源码只读诊断（60 秒更新周期、`Info.MemPct` evaluator、`alerts.triggered` 更新钩子写入 `alerts_history`）；下一步只做系统记录、alerts 记录和 Hub 日志的同窗只读核对，不把空记录直接当成已定位的 Beszel 漏报。
