...
| 项 | 公司电脑 | 本地电脑（Windows + WSL2） |
| --- | --- | --- |
| 主要工作 | 生产环境采集、汇报、与 leader 沟通 | Beszel PoC、文档整理、调研 |
| 生产基线采集 | ✅ reports/production-environment.txt（不入库） | — |
| WSL2 + Docker PoC 环境 | — | ✅ Ubuntu 26.04.1 / Docker 29.1.3 |
| Beszel 0.19.0 Hub/Agent | 待确认是否已部署 | ✅ 已上线，认证通过 |
| deploy/beszel/.env | 需按 docs/11 重新生成（不入库） | 已配置（不入库） |
...
- 2026-09-17 [本地电脑] 完成汇报讲稿《服务器资源保护-现成策略优先-讲稿.md》（21 页，现成策略优先、自研仅作备选）。
- 2026-09-17 [本地电脑] 建立 git 仓库基线并首次提交，配置 GitHub 远程（github.com/fcsfang/server-resource-guardian）。
...
注意：以下内容刻意不入库，换电脑后需各自重建：
- `deploy/beszel/.env`（本机 PoC 凭据，按 docs/11 重新走 Add System 流程）
- `reports/*`（本机环境采集产物，含生产信息，仅本机保留）
- WSL2 内的 Docker 容器与 Beszel 指标数据（运行态，不属于仓库）
