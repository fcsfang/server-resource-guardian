...
本地功能已完成：CPU、内存和磁盘风险均进入同一常驻主链，只读采集、保护对象、技术上的短期精确授权、执行前复核、最多一个目标、恢复检查、通知持久化和审计已接通。`graceful_stop` 只发送一次正常退出信号，超时不强杀、不重试、不选第二个目标。用户对本地可丢弃验收已给予持续业务授权，后续不再重复询问；但系统内部仍为每个完整容器 ID 生成短期、可消费一次的技术授权，不放宽保护边界。磁盘当前只能释放 Guardian 自有预留空间，不会删除业务数据。本地版本仍不是生产版本。

进入 x86_64 非生产验证前，真实管理员通知渠道、收件人授权和送达测试目标仍待负责人确认；这不是本地代码已完成的功能证明。

## 下一步唯一交付物

确定真实管理员通知渠道和收件人，完成 Beszel 测试通知的实际送达；同时提交并获得批准的 [x86_64 非生产验证申请包](docs/nonproduction-x86_64-validation-request.md)，确认主机、时间窗口和回滚负责人。获批后先运行 x86_64 只读准入检查，再安装只读观察版本。获批前不连接非生产或生产服务器。
