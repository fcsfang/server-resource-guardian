...
- 内存真实止损的本地目标已准备但未启动：虚拟机 `guardian-t11-matrix`，容器 `guardian-memory-target-approval`，完整 ID `1b1826ff912588135cb1e31b109fff9b71e53cdeed6c35503ea946914e62fd0d`，拟执行动作仅为一次 `graceful_stop`。当前状态为 `created`，在用户明确授权前不会启动压力或执行动作。
