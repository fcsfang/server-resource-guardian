...
# session, individually timed. Verified in EXP-088 scenario 7 (A-B-A) and the
# 2026-09-23 benefit run - run against the TARGET inside the collapse window.
# Usage:  powershell -File ops_chain.ps1 -Label run1 -StopTarget ben-swarm-3
# Reuse via env:  $env:LAB_TARGET = "user@host";  $env:LAB_KEY = "path\to\key"
param(
  [string]$Label = "run",
  [string]$StopTarget = "ben-swarm-3",
