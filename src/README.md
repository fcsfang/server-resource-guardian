...
PG-P0-07 新增 [`guardian_runtime.py`](guardian_runtime.py) 和 [`deploy/guardian/`](../deploy/guardian/)：提供 systemd `READY=1`/watchdog 通知、原子 readiness 文件、非 root Observer unit、独立 slice、有界日志和本地资源起始值。当前只完成模板静态校验与临时 observe smoke；没有安装/启用持久服务，资源参数不是生产校准值，24 小时 soak 和故障注入仍待完成。
